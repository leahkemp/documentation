---
title: "Benchmarking pipeline resources - per rule - production cluster"
author: "Leah Kemp"
date: "9/21/2020"
...
This document aims to plot and analyse the results of resource benchmarking tests for our genomic pipelines on the ESR cluster (production). See related docs here: [resource benchmarking per rule production cluster](../resource_benchmarking_per_rule_production_cluster.md)
...
genmod_score_eightsample_data <- genmod_score_data %>%
  dplyr::filter(no_samples == "8")
genmod_models_eightsample_data <- genmod_models_data %>%
  dplyr::filter(no_samples == "8")
...
SnpSift_filter_proband_plot <- ggplot2::ggplot(data = SnpSift_filter_proband_data, aes(x = threads_per_rule, y = minutes, group = filepath_benchmarking_file, colour = no_samples)) +
  geom_point(alpha = my_alpha) +
  geom_line(alpha = my_alpha) +
  labs(title = "SnpSift_filter_proband", x = xlab, y = ylab, colour = legendtitle) +
  scale_x_continuous(breaks=c(1, 2, 4, 8, 16, 32), limits=c(1, 32), trans='log10') +
  scale_y_continuous(limits=c(0,3), 
    sec.axis = sec_axis(~ ./60, name = secondaxis)) +
  geom_vline(xintercept = 1, linetype = "dashed")
...
genmod_models_plot <- ggplot2::ggplot(data = genmod_models_data, aes(x = threads_per_rule, y = minutes, group = filepath_benchmarking_file, colour = no_samples)) +
  geom_point(alpha = my_alpha) +
  geom_line(alpha = my_alpha) +
  labs(title = "genmod_models", x = xlab, y = ylab, colour = legendtitle) +
  scale_x_continuous(breaks=c(1, 2, 4, 8, 16, 32), limits=c(1, 32), trans='log10') +
...
grid.arrange(SnpSift_filter_proband_plot,
             genmod_score_plot,
             ncol = 1, nrow = 2)
```
...
trim_galore_pe_eightsample_node_plot <- ggplot2::ggplot(data = trim_galore_pe_eightsample_data, aes(x = threads_per_rule, y = minutes, group = filepath_benchmarking_file, colour = node)) +
  geom_point(alpha = my_alpha) +
  geom_line(alpha = my_alpha) +
  labs(title = "trim_galore_pe - eight samples", x = xlab, y = ylab, colour = legendtitle) +
  scale_x_continuous(breaks=c(1, 2, 4, 8, 16, 32), limits=c(1, 32), trans='log10') +
  scale_y_continuous(limits=c(0,500), 
    sec.axis = sec_axis(~ ./60, name = secondaxis))
...
grid.arrange(gatk_HaplotypeCaller_single_eightsample_node_plot,
             trim_galore_pe_eightsample_node_plot,
             ncol = 1, nrow = 2)
```
